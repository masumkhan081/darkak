...
export default function CustomInput({
  ...
  return (
    <div className="flex items-center rounded-md border border-slate-300 w-full h-full font-sans text-base">
      {startIcon && (
        <span className="bg-slate-200 h-full py-0.25 px-0.25 border-r rounded-r-sm border-slate-300">
          {startIcon}
        </span>
      )}
      ...
      <input
        ...
        required={true}
        placeholder={ph}
        className="flex-grow py-0.25 px-0.38 outline-none  h-fit rounded overflow-hidden"
        value={value}
